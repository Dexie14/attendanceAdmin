...
// Sample data type
export type AttendanceDataItem = {
  _id: string;
  staffId?: string;
  staffName?: string;
  location?: string;
...
const sampleData: AttendanceDataItem[] = [
  {
    _id: "1",
    staffId: "9394hu94",
...
const AttendanceTable = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<AttendanceDataItem | null>(
    null
  );

  const renderRow = (item: AttendanceDataItem, index: number) => {
    const handleRowClick = (row: AttendanceDataItem) => {
      setSelectedRow(row);
    };
  ...
  return (
    <div>
      ...
      />
      {
        ...
        >
          {selectedRow && <AttendanceDetail selectedRow={selectedRow} />}
        </ReusableDialog>
      }
    </div>
  );
