...
const sampleData: LeaveDataItem[] = [
  {
    _id: "1",
    staffId: "9394hu94",
...
const LeaveTable = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<LeaveDataItem | null>(null);

  const renderRow = (item: LeaveDataItem, index: number) => {
    const handleRowClick = (row: LeaveDataItem) => {
      setSelectedRow(row);
    };
  ...
  return (
    <div>
      ...
      />
      {
        ...
        >
          {selectedRow && <LeaveDetails selectedRow={selectedRow} />}
        </ReusableDialog>
      }
    </div>
  );
